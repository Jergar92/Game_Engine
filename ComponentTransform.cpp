...
ComponentTransform::ComponentTransform(GameObject* my_go, float3 scale, Quat rotation, float3 position ) :Component(my_go), scale(scale), rotation(rotation), position(position)
{
...
float4x4 ComponentTransform::GetTransposedMatrix()const
{
...
bool ComponentTransform::SaveComponent(JSONConfig & config) const
{
	bool ret = true;
...
bool ComponentTransform::LoadComponent(const JSONConfig & config)
{
	bool ret = true;
